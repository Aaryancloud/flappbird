...
def save_high_score(new_high_score):
    try:
        with open(HIGH_SCORE_FILE, "w", encoding="utf-8") as handle:
            handle.write(str(new_high_score))
    except OSError:
        pass
...
def draw_pipes(pipes_to_draw):
    for top, bottom, _ in pipes_to_draw:
...
def check_collision(bird_rect, pipes_to_check):
    for top, bottom, _ in pipes_to_check:
...
def main():
    global screen, clock, font, large_font, JUMP_SOUND, SOUND_LOADED
    global BACKGROUND_MUSIC_LOADED, bird_image, bird_radius, BIRD_WIDTH, BIRD_HEIGHT
    global bird_y, bird_velocity, pipes, score, high_score, time_since_last_pipe, game_active
    global running, pipe_frequency
    ...
    try:
        sound_file_path = resolve_asset_path("jump_sound.mp3.mpeg")
    ...
    try:
        bird_image_path = resolve_asset_path("bird.png")
    ...
    while running:
        ...
        restart_button = None
        if not game_active:
            restart_button = pygame.Rect(WIDTH // 2 - 75, HEIGHT // 2 + 120, 150, 60)
        ...
        if game_active:
            ...
            screen.blit(high_score_text, (10, 42))
        else:
            ...
            screen.blit(high_score_text, (WIDTH // 2 - high_score_text.get_width() // 2, HEIGHT // 2 + 55))
